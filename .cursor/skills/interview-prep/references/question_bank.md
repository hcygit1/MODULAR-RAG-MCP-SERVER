...
>
> **与代码对齐（2026-03）**：默认存储为 SQLite 统一库 + FTS5；MCP 共 **4** 个 tools；无 `get_document_summary`；文档更新靠 `source_doc_id` + replace；Ragas 可能未在工厂中实现。题中若出现旧版「Chroma+独立 BM25 pickle+DocumentManager」表述，面试官应自行改写为当前实现。
...
7. 这个系统里数据主要存在哪里？（向量、全文索引、chunk 文本、图片）SQLite 统一存储与拆库方案各有什么取舍？
...
6. 系统里用了哪些存储？（默认 SQLite 统一库 + 文件完整性库）向量与 FTS5、图片表、ingestion_history 各自是什么关系？
...
3. 你说负责过文档更新与幂等，那同一文档修改后再入库，怎样避免旧 chunk 残留？`source_doc_id` 和 `replace_document_chunks` 分别起什么作用？
...
4. CLI `ingest.py` 的 `process` 和 MCP `ingest_document_normal` 的 `process_document` 在完整性检查与加载上的分工有什么不同？
...
10. 配置里写了 `evaluation.backends: [ragas, custom]`，当前 Evaluator 工厂里 ragas 是否已落地？你们实际跑的是哪条评估链路？
...
### 【A — 检索架构】关键词：Hybrid Search / RRF / FTS5 / 向量检索 / 混合检索 / 父聚合

A1. RRF 公式是什么？k 值怎么选？为什么不用线性加权？
A2. BM25 相比 TF-IDF 改进了什么？BM25 里的 b 参数和 k1 参数各控制什么？
A3. 当 `vector_store.backend=sqlite` 且 `sparse_backend=fts5` 时，稀疏这一路索引和数据存在哪里？重新摄取文档时全文索引如何与 chunk 保持一致？
...
A10. `aggregate_by_parent` 为 true 时，检索结果如何从子块变成「父级」正文？`parent_aggregate_top_m` 解决什么问题？
...
C7. `splitter_strategy: heading` 时，父级与子级标题级别由哪些配置项控制？`parent_id` 在检索侧怎么用？
...
C10. 源文件更新后重新入库，仅靠 upsert 不写删除可能会有什么问题？你们用什么机制保证旧 chunk 不残留？
...
D6. MCP 当前注册了哪几个 tool？`ingest_document_normal` 与 `ingest_document_mineru` 分别在什么场景下选用？
...
### 【E — 存储 & 幂等性】关键词：SQLite / FTS5 / source_doc_id / 幂等 / SHA256 / 存储

E1. chunk 的 id 在 pipeline 里大致怎么生成？`source_doc_id` 与 `document.id` 是什么关系？文档改短后为何要按文档删旧块？
E2. `FileIntegrityChecker` 默认把处理记录存在哪里？和向量库里的 chunk 是什么关系？
E3. MCP 工具里 `force=true/false` 分别如何影响完整性跳过？与 CLI `--force` 的语义是否一致？
E4. 为什么说「sqlite + fts5」时稀疏索引必须与 chunk 同库？对部署与备份有什么影响？
E5. `collection_name` 在 ingest 与 query 里如何传递？默认集合从哪里来？
E6. MinerU 路径与普通 PDF 路径入库后，metadata 或块结构可能有哪些差异？
E7. SQLite 后端下图片二进制存在哪里？检索返回里如何带上图片（与 `image_refs` 的关系）？
E8. 若换用 Chroma/Qdrant 作 vector_store，文档级 `delete_by_source_doc_id` 若未实现会发生什么？
...
F4. 当前仓库里 Ragas 评估是否已在 EvaluatorFactory 中实现？若未实现，你们用什么方式做离线评估（脚本与模块）？
F5. Trace 是怎么实现的？Ingestion 的 5 个阶段 Trace 各记录了哪些数据？
F6. Dashboard 里的 Query Trace 能看到什么信息？Dense/Sparse 召回结果是怎么对比展示的？
F7. Golden Test Set 路径在配置里哪里声明？EvalRunner / content_evaluator 大致如何消费它？
...
G4. 统一 SQLite 存储与「向量库 + 外部搜索引擎」拆分相比，各有什么工程权衡？什么规模会考虑换 backend？
