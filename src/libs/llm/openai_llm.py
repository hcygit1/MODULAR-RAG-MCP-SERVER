...
class OpenAILLM(BaseLLM):
    ...
    def chat(self, messages: List[Dict[str, str]]) -> str:
        ...
        try:
            ...
            raise RuntimeError(
                f"OpenAI API 调用失败 (provider={self._provider}, model={self._model}): {error_msg}"
            ) from e
        except urllib.error.URLError as e:
            raise RuntimeError(
                f"OpenAI API 网络错误 (provider={self._provider}, model={self._model}): {str(e)}"
            ) from e
        except Exception as e:
            raise RuntimeError(
                f"OpenAI LLM 调用失败 (provider={self._provider}, model={self._model}): {str(e)}"
            ) from e
